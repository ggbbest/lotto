...
年会즉석로또程序，3D 球体즉석로또，支持奖品信息配置，参与즉석로또人员信息`Excel`导入，추첨결과`Excel`导出
...
前端즉석로또界面通过`Three.js`实现 3D 즉석로또球，引用了`Three.js`的官方 3D 示例

## 功能描述：

1. 可将추첨결과进行保存实时下载到 excel 中🎉
2. 已抽取人员不在参与抽取，抽中的人员不在现场可以重新抽取🎁
3. 刷新或者关掉服务器，会保存当前已抽取的数据，不会进行数据重置，只有点击界面上的重置按钮，才能重置즉석로또数据🧧
4. 每次抽取的奖品数目可配置🎈
5. 抽取完所有奖品后还可以继续抽取특별상(例如：现在抽取红包，追加的奖品等)，此时默认一次抽取一个🧨
...
즉석로또用户信息在**`server/data/user.xlsx`**文件中，按文件格式进行填充，不能修改文件名和列头
...
| 参数  | 值类型 | 描述                                                         |
| ----- | ------ | ------------------------------------------------------------ |
| type  | Number | 奖品类型，唯一标识，0 是默认특별상的占位符，其它奖品不可使用 |
| count | Number | 奖品数量                                                     |
| text  | String | 奖项名称                                                     |
| title | String | 奖品描述                                                     |
...
**EACH_COUNT**用于配置每次的즉석로또个数，与 prizes 奖品一一对应，例如上面的奖品配置对应的즉석로또个数配置如下：

```js
const EACH_COUNT = [1, 1, 5];
```

如上配置，表示一次抽取的奖品个数顺序为：특별상每次抽一个，特等奖每次抽一个，一等奖每次抽五个
